...
bool HMC5883L::begin()
{
	...
	//Serial.println( fastRegister8(HMC5883L_REG_IDENT_A), HEX );
	//Serial.println( fastRegister8(HMC5883L_REG_IDENT_B), HEX );
	//Serial.println( fastRegister8(HMC5883L_REG_IDENT_C), HEX );
...
uint8_t HMC5883L::readRegister8(uint8_t reg)
{
    ...
    Wire.beginTransmission(HMC5883L_ADDRESS);
    Wire.requestFrom(HMC5883L_ADDRESS, 1);
    maxLoop = 0; // At 240Mhz Wait max 8ms
	while( maxLoop < 2000000 && !Wire.available())
	{ maxLoop++; };
    #if ARDUINO >= 100
        value = Wire.read();
    #else
